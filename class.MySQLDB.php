...
class MySQLDB {
  ...
  public function getHistoryLatest() {
    ...
    if ($this->mysql_module_loaded == true) {
      if ($this->db_connection != null) {
        $result = array();
        $query = "SELECT DISTINCT s.entry, CASE WHEN s.entry LIKE '1239%' THEN '*' ELSE '' END AS hightlight, s.orgName, s.comment, h.path FROM b19_se.history AS h JOIN b19_se.swish as s ON s.entry = h.entry WHERE s.entry IS NOT NULL AND h.dt > date_sub(NOW(), INTERVAL 24 HOUR) GROUP BY h.entry ORDER BY h.dt DESC LIMIT 25;";
